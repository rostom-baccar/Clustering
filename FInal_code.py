...
def load_txt_data (name,i) :
    ...
    plt.scatter(f0,f1,s=8)
    plt.title("Données Mystères "+str(i))
    plt.show()
    d={}
    d["data"] = datanp
    d["f0"] = f0
...
def kmeans_iteration( loaded_data , k ):
    ...
    plt.title("Kmeans Clustering / k = "+str(k))
    plt.show()
...
def kmedoids_iteration ( loaded_data , k ):
    ...
    plt.title("KMedoids Clustering / k = "+str(k))
    plt . show ()
...
def agglomerative_iteration (loaded_data , linkage, distance=None, k=None):
    ...
    plt.scatter(loaded_data["f0"],loaded_data["f1"],c = labels,s = 8 )
    plt.title("Agglomératif / linkage "+str(linkage)+" / distance = "+str(distance)+" / k = "+str(k))
    plt.show()
...
def dbscan_iteration (loaded_data , eps , min_samples):
    ...
    plt.title ("DBSCAN Clustering | eps = "+str(int(eps))+" | min_samples = "+str(min_samples)+" | k = "+str(k))
    plt.show()
...
def hdbscan_iteration (loaded_data , min_samples):
    ...
    plt.scatter(loaded_data["f0"],loaded_data["f1"],c = labels,s = 8 )
    k = len(set(labels)) - (1 if -1 in labels else 0)
    plt.title ("HDBSCAN Clustering | min_samples = "+str(min_samples)+" | k = "+str(k))
    plt.show()
    ...
    sc_sil = metrics.silhouette_score(loaded_data["data"], labels, metric='euclidean')
    print("score is "+str(sc_sil))
...
def dbscan_evaluation_graph(loaded_data , max_min_samples):
    ...
    return [best_eps,best_min_samples,best,runtime_list,k_list]

#%%
#dbscan_evaluation_graph(loaded_d31,50)
l = dbscan_evaluation_graph(loaded_d31,20)
l1 = hdbscan_evaluation_graph(loaded_d31,20)
#%%
plt.plot(l[4],l[3],label ='Runtime HDBSCAN (10**-1 s))')
plt.plot(l[4],l1[2],label ='Runtime DBSCAN (10**-1 s))')
plt.title("Runtime")
plt.legend()
#%%
def hdbscan_evaluation_graph(loaded_data , max_min_samples):
    ...
    return [best_min_samples,best,runtime_list]
...
for filename in os.listdir(directory):
    ...
    if os.path.isfile(f):
        i+=1
        print(filename)
        load_txt_data(filename,i)
#%%
#Loading mystery data
loaded_dm1 = load_txt_data("x1.txt")
loaded_dm2 = load_txt_data("x2.txt")
loaded_dm3 = load_txt_data("x3.txt")
loaded_dm4 = load_txt_data("x4.txt")
loaded_dm5 = load_txt_data("y1.txt")
loaded_dm6 = load_txt_data("zz1.txt")
loaded_dm7 = load_txt_data("zz2.txt")
#%%
def mystery_data_cluster(loaded_data):
    ...
    l_kmeans=kmeans_evaluation_graph('kmeans', loaded_data, max_k)
    best_kmeans=l_kmeans[0]
    best_score_kmeans=l_kmeans[1]
    scores.append(best_score_kmeans)
    kmeans_iteration(loaded_data, best_kmeans)
    
    l_kmedoids=kmeans_evaluation_graph('kmedoids', loaded_data, max_k)
    best_kmedoids=l_kmedoids[0]
    best_score_kmedoids=l_kmedoids[1]
    scores.append(best_score_kmedoids)
    kmedoids_iteration(loaded_data, best_kmedoids)

    l_agg=agglomerative_evaluation_graph_distance(loaded_data, linkage, 200000,10000000,500000)
    best_distance=l_agg[0]
    best_score_agg=l_agg[1]
    scores.append(best_score_agg)
    print('best distance is '+str(best_distance))
    agglomerative_iteration(loaded_data, linkage, best_distance)
    
    dbscan_list=dbscan_evaluation_graph(loaded_data,max_min_samples)
    best_eps=dbscan_list[0]
    best_min_samples_dbscan=dbscan_list[1]
    best_score_dbscan=dbscan_list[2]
    scores.append(best_score_dbscan)
    dbscan_iteration(loaded_data,best_eps,best_min_samples_dbscan)
    
    hdbscan_list=hdbscan_evaluation_graph(loaded_data,max_min_samples)
    best_min_samples_hdbscan=hdbscan_list[0]
    best_score_hdbscan=hdbscan_list[1]
    scores.append(best_score_hdbscan)
    hdbscan_iteration(loaded_data,best_min_samples_hdbscan)
    
    print("scores :"+str(scores))
    
    return None

mystery_data_cluster(loaded_dm4) 
